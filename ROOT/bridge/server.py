...
@app.get("/health")
async def health():
    """Quick liveness check for the frontend startup screen."""
    ready = _stack is not None
    return {"status": "ok" if ready else "starting", "ready": ready}


@app.get("/session/bootstrap_status")
async def bootstrap_status():
    """Polling endpoint for the frontend startup screen.

    Returns 'ready' once the stack is initialised and available.
    """
    if _stack is None:
        return {"status": "starting", "ready": False}
    has_session = _stack.orchestrator.session is not None
    return {"status": "ready", "ready": True, "has_session": has_session}
...
# Cancellation flag — checked between pipeline phases.
...
@app.post("/dev/pipeline/cancel")
async def cancel_pipeline():
    """Request cancellation of a running homework/reflective pipeline.

    Sets a flag that can be checked between pipeline phases.  The actual
    interruption depends on the pipeline implementation cooperating with
    the flag.  Returns immediately.
    """
...
@app.post("/memory/ltmm/knowledge/notebook")
async def create_notebook_entry(req: NotebookCreateRequest):
    """Create a new notebook entry via the frontend editor."""
    stack = _require_stack()
    _require_session(stack)
    try:
        ...
        entry = NotebookEntry(
            ...
            source_mode="frontend",
        )
        stack.memory.knowledge.notebook.write(entry)
        log.info("Notebook entry created: %s", entry.note_id)
        return {"status": "ok", "note_id": entry.note_id}
    except Exception as exc:
        log.exception("Notebook write failed.")
        raise HTTPException(status_code=500, detail=str(exc))
...
class HardcodedEditRequest(BaseModel):
    content: str
    persist: bool = False


# Session-level overrides for hardcoded entries (non-persistent by default).
_hardcoded_session_overrides: dict[str, str] = {}


@app.post("/memory/ltmm/identity/hardcoded/{entry_id}")
async def update_hardcoded(entry_id: str, req: HardcodedEditRequest):
    """Update a hardcoded identity entry.

    The HardcodedStore is immutable by design.  Session-only edits are
    stored in an in-memory override dict that is lost on restart.
    Persistent edits attempt to mutate the underlying entry object
    (best-effort — the store may reject it).
    """
    stack = _require_stack()
    _require_session(stack)
    if req.persist:
        # Best-effort: try to mutate the entry object in-place.
        entry = None
        for e in stack.memory.identity.hardcoded.get_all():
        ...
        try:
            object.__setattr__(entry, "content", req.content)
        except Exception:
            entry.content = req.content  # type: ignore[attr-defined]
        log.info("Hardcoded entry %s updated (persistent).", entry_id)
    else:
        _hardcoded_session_overrides[entry_id] = req.content
        log.info("Hardcoded entry %s overridden (session only).", entry_id)
    return {"status": "ok", "entry_id": entry_id, "persist": req.persist}
...
@app.post("/memory/ltmm/unsolved/{entry_id}/skip_dream")
async def skip_dream_candidate(entry_id: str):
    """Mark an unsolved entry as skipped for dream processing.

    Resolves the entry with a note indicating it was skipped by the user
    during dream candidate selection.
    """
    stack = _require_stack()
    _require_session(stack)
    try:
        stack.memory.thoughts.unsolved_buffer.resolve(
            entry_id, note="Skipped by user during dream candidate selection.")
        return {"status": "ok", "entry_id": entry_id, "action": "skipped"}
    except Exception as exc:
        raise HTTPException(status_code=404, detail=str(exc))
...
class E17LearningRequest(BaseModel):
    enabled: bool = True


@app.post("/dev/reward/e17_learning")
async def toggle_e17_learning(req: E17LearningRequest):
    """Toggle E17 parameter learning on/off for this session.

    Stores the flag on the session object so dispatch can skip or include E17.
    """
    stack = _require_stack()
    session = _require_session(stack)
    session.e17_learning_enabled = req.enabled
    log.info("E17 parameter learning → %s", "enabled" if req.enabled else "disabled")
...
class SleepActivateRequest(BaseModel):
    mode: str = "triage"  # triage | rem | dream


@app.post("/dev/sleep/activate")
async def activate_sleep(req: SleepActivateRequest):
    """Activate a specific sleep mode (triage, rem, dream)."""
    stack = _require_stack()
    if stack.orchestrator.session is None:
        stack.orchestrator.open_session()
    cmd = "/sleep" if req.mode == "triage" else f"/sleep {req.mode}"
    loop = asyncio.get_event_loop()
    try:
        result = await loop.run_in_executor(None, lambda: _run_turn(stack, cmd))
    except Exception as exc:
        log.exception("Sleep activate (%s) failed.", req.mode)
        raise HTTPException(status_code=500, detail=str(exc))
    return safe_json(result) or {"status": "activated", "mode": req.mode}


@app.post("/dev/sleep/exit")
async def exit_sleep():
    """Exit sleep mode and return to waking state."""
    stack = _require_stack()
    session = _require_session(stack)
    # Clear any sleep-related session flags
    session.session_mode = "regular"
    log.info("Sleep mode exited — session_mode reset to regular.")
    return {"status": "ok", "session_mode": "regular"}
...
# ---------------------------------------------------------------------------
# Map workspace — mutation endpoints
# ---------------------------------------------------------------------------
...
@app.post("/map/atoms/{atom_id}")
async def update_atom(atom_id: str, req: AtomUpdateRequest):
    """Update truth value and/or attention value on an atom."""
    stack = _require_stack()
    engine = _find_atomspace_engine(stack)
    if engine is None:
        raise HTTPException(status_code=404, detail="AtomSpace engine not found.")
    try:
        from zados.cognitive_engines.cognitools.atomspace_engine import (
            AttentionValue,
            TruthValue,
        )
        atom = engine._atoms.get(atom_id)
        if atom is None:
            raise HTTPException(status_code=404, detail=f"Atom {atom_id!r} not found.")
        if req.strength is not None or req.confidence is not None:
            old_tv = getattr(atom, "truth_value", None) or TruthValue()
            new_tv = TruthValue(
                strength=req.strength if req.strength is not None else old_tv.strength,
                confidence=req.confidence if req.confidence is not None else old_tv.confidence,
            )
            engine.update_truth_value(atom_id, new_tv)
        if req.sti is not None:
            old_av = getattr(atom, "attention_value", None) or AttentionValue()
            new_av = AttentionValue(sti=req.sti, lti=old_av.lti)
            engine.update_attention_value(atom_id, new_av)
        return {"status": "ok", "atom_id": atom_id}
    except HTTPException:
        raise
    except Exception as exc:
        log.exception("Atom update failed.")
        raise HTTPException(status_code=500, detail=str(exc))


@app.post("/map/atoms/{atom_id}/delete")
async def delete_atom(atom_id: str):
    """Remove an atom (with cascade) from the AtomSpace."""
    stack = _require_stack()
    engine = _find_atomspace_engine(stack)
    if engine is None:
        raise HTTPException(status_code=404, detail="AtomSpace engine not found.")
    try:
        removed = engine.remove_atom(atom_id, cascade=True)
        if not removed:
            raise HTTPException(status_code=404, detail=f"Atom {atom_id!r} not found.")
        return {"status": "ok", "atom_id": atom_id, "removed": True}
    except HTTPException:
        raise
    except Exception as exc:
        log.exception("Atom delete failed.")
        raise HTTPException(status_code=500, detail=str(exc))
...
async def add_link(req: AddLinkRequest):
    """Add a typed link between two atoms in the AtomSpace."""
    stack = _require_stack()
    engine = _find_atomspace_engine(stack)
    if engine is None:
        raise HTTPException(status_code=404, detail="AtomSpace engine not found.")
    try:
        ...
        if req.source not in engine._atoms:
            raise HTTPException(status_code=404, detail=f"Source atom {req.source!r} not found.")
        if req.target not in engine._atoms:
            raise HTTPException(status_code=404, detail=f"Target atom {req.target!r} not found.")
        # Resolve link type
        try:
            link_type = AtomType(req.type)
        except ValueError:
            link_type = AtomType.INHERITANCE_LINK
        tv = TruthValue(strength=req.weight, confidence=0.5)
        atom = engine.add_link(link_type, (req.source, req.target), truth_value=tv,
                               source_engine="frontend")
        return {"status": "ok", "link_id": getattr(atom, "atom_id", "")}
    except HTTPException:
        raise
    except Exception as exc:
        log.exception("Add link failed.")
        raise HTTPException(status_code=500, detail=str(exc))


@app.post("/map/save")
async def save_atomspace():
    """Persist the AtomSpace to its backing store."""
    stack = _require_stack()
    engine = _find_atomspace_engine(stack)
    if engine is None:
        raise HTTPException(status_code=404, detail="AtomSpace engine not found.")
    try:
        # Look for a CognitoolsDataStore on the stack
        store = getattr(getattr(stack, "memory", None), "cognitools_data", None)
        if store is None:
            return {"status": "ok", "saved": False, "reason": "No backing store configured."}
        saved = engine.persist_to_store(store)
        log.info("AtomSpace persisted: %s", saved)
        return {"status": "ok", "saved": saved}
    except Exception as exc:
        log.exception("AtomSpace save failed.")
        raise HTTPException(status_code=500, detail=str(exc))
