ZA-DOS Memory Layer Architecture
Technical Specification v2.0
Updated: March 23, 2026  |  Status: Code-Verified (6015 tests)
...
3 (STMM, MTMM, LTMM)
STMM components
10 dataclass containers
MTMM subsystems
3 (RawInteractionLogger, MidTermProcessingEngine, ContextProcessor)
LTMM flat store
LTMMStore with TF-IDF semantic index
LTMM namespaces
3 (Identity: 5 stores, Knowledge: 7 stores, Thoughts: 4 stores)
Specialized logs
8 domain-specific log subsystems
Journal system
JournalStore with trigger-based writing + engine annotations
Retrieval system
RetrievalRouter + MemoryContrast + ScopeFilter
Pipeline scopes
10 predefined (Regular, M1-M5, Homework, Reflective, REM, Dream)
Tag taxonomy
4 categories (identity, cognitive, pipeline, domain)
Search backend
TF-IDF cosine similarity (no external ML dependencies)
Facade class
MemoryLayer (coordinates all tiers and managers)
...
STMM holds the last two user inputs and last two AI outputs plus all current-cycle analysis results and flags. Content is overwritten on each new processing cycle via begin_cycle() — there is no accumulation. The active message buffer preserves messages across cycles; analysis components are reset.
1.2 Capacity
STMM is bounded by design. ActiveMessageBuffer uses FIFO with window size 2 per speaker (SpeakerID.USER and SpeakerID.SYSTEM). When a third user message arrives, the oldest is evicted (but has already been compressed to MTMM by MemoryExitCompressor).
2. STMMStore
...
CorticalReflectionLog()
brain_process_tracker
BrainProcessTracker
BrainProcessTracker()
reward_evaluation
RewardEvaluationResults
RewardEvaluationResults()
cephalic_liquid_logger
...
turn_index
int
Cycle counter at time of message
...
pressure_type
str
"stable"  (stable | escalating | de-escalating)
...
Type
Default
user_emotion_signals
Dict[str, float]
{}
system_emotion_state
Dict[str, float]
{}
emotional_delta
Dict[str, float]
...
nt_concentrations
Dict[str, float]
All 12 NTs, normalized [0, 1]
receptor_occupancy
Dict[str, float]
Subtype-level receptor states
oscillatory_bands
Dict[str, float]
Delta, Theta, Alpha, Beta, Gamma, HFO
neurosymbolic_metrics
Dict[str, float]
motivation, empathy, cognitive_rigidity, fatigue, precision, openness, anxiety, social_engagement + 3 more
modulation_signals
Dict[str, Any]
Active stochastic modulation
extractor_state
Dict[str, Any]
Extractors 1–4 outputs
...
Intention
Primary only
Sub-intentions dropped
Emotion vector
Lossless
User + system (prefixed with sys_)
Neurochemical snapshot
Lossless
Concentration values preserved
Reward scores
Domain-level only
Subscores dropped
Memory contrast matches
Significance-filtered
Only matches > 0.30 threshold
Engine execution trace
Compressed
Engine ID list + anomalies only
Verbal reflection
Lossless
Full VT monologue preserved
Emotional significance
Computed
max(saturation_levels) from emotion detection
...
Retention Policy:
MTMM persists for the duration of a session. At session end (or during REM processing), the MemoryConsolidationEngine evaluates MTMM contents for LTMM promotion. Content that doesn’t meet promotion criteria is either archived or discarded.
2. MTMMStore
...
Ordered MemoryPacket storage
Trend tracker
MidTermProcessingEngine
Session-level trend accumulation
Semantic index
ContextProcessor
TF-IDF search + validation + compression
...
List[int]
Per-turn contradiction counts
emotional_trajectory
List[Dict[str, float]]
Emotion states over time
reward_trajectories
Dict[str, List[float]]
Per-domain reward series
intention_pattern
List[str]
Sequence of primary intentions
nt_trajectories
Dict[str, List[float]]
NT concentration time series
contradiction_trend
str
...
reward_trend
Dict[str, str]
Per-domain: improving | stable | declining
intention_shift
bool
Whether intent changed from previous majority
...
1.1 Retention Policy
LTMM content is persistent but not permanent. The MemoryRelevanceHeuristicsEngine periodically evaluates stored content and demotes entries whose relevance decays below threshold. Identity-relevant entries are protected and never demoted.
1.2 Access Pattern
Read access via MemoryContrast (weighted MTMM/LTMM blend) or RetrievalRouter (namespace routing). Write access controlled exclusively by MemoryImplementationManager.
2. LTMMStore (Flat)
...
MemoryPacket
(required)
The stored memory packet
granularity
str
"semantic"
verbatim | semantic | symbolic
relevance_score
float
1.0
...
CRITICAL, SEVERE, IDENTITY, PARADOX, UNRESOLVED
VERBATIM
Yes if IDENTITY
Low trust weight
< 0.4
...
> 0.85
merge
Near-duplicate — reinforce existing entry, don't write
≥ 0.60
reinforce
Cross-link; if both semantic + same pattern → promote to SYMBOLIC
< 0.60
write
Unique content — write new entry
...
Recency
0.30
exp(-λ * hours_since_access), λ = ln(2)/168 (1-week half-life)
Frequency
0.20
1 - exp(-0.23 * retrieval_count), saturates at ~10
Emotion
0.20
packet.emotional_significance (raw value)
Utility
0.20
entry.utility_score (updated via feedback)
Coherence
0.10
0.5 (placeholder)
...
entry_id
str
Unique identifier (e.g., axiom_curiosity)
content
str
The axiom/value/constraint text
category
str
axiom | value | constraint | system_prompt
tags
List[str]
Classification tags
...
Axioms
4
axiom_curiosity, axiom_honesty, axiom_care, axiom_identity_continuity
Values
4
value_intellectual_humility, value_relational_attunement, value_depth_over_performance, value_ethical_clarity
Constraints
3
constraint_no_deception, constraint_no_identity_override, constraint_emotional_safety
Personality
5
personality_voice_core, personality_rhythm, personality_intellectual_engagement, personality_emotional_register, personality_disagreement
System
1
system_nature
...
memory_type
str
"" (experience | relationship | self_model | event)
tags
List[str]
[]
version
...
conclusion_type
str
"" (value | lesson | self_insight | boundary)
source_refs
List[str]
[]
confidence
...
REGULAR (REGULAR | REFLECTION | COMMENT)
content
str
""
parent_entry_id
...
source_pipeline
str
"" (reflective_mode | peer_review | rem)
...
validation_status
str
"pending" (pending | validated | contradicted)
cross_links / knowledge_map_refs
List[str]
[]
reinforcement_count
int
...
str
""
subject_category / source_mode
str
""
related_lessons / related_questions
List[str]
[]
nt_snapshot
Dict[str, float]
...
str
""
subject_category / source_engine
str
""
blocking_reason
str
...
source
str
"" (self_generated | user_triggered | engine_flagged)
...
nodes
List[KnowledgeNode]
Graph nodes (see below)
links
List[KnowledgeLink]
Graph edges (see below)
contributing_lessons
List[str]
Lesson IDs that built this map
atomspace_ref
Optional[str]
Link to Engine 9 AtomSpace
...
str
""
emotion_tag / emotion_trigger_type
str
""
nt_snapshot
Dict[str, float]
...
pipeline_phase
str
"" (phase4_thinking | verbalized_thinking)
session_id / source_turn_ref
str
""
reviewed
bool
...
LTMM_THRESHOLD
LTMM promotion event threshold
REM_COMPLETE
End of REM processing
INNOVATION_FLAG
Innovation domain flag triggered
DEV
...
List[str]
Patterns spanning sessions
parallel_concepts
List[str]
Analogous concepts identified
novelty_flags
List[str]
...
learning_type
str
"" (contextual | reward | reflective | recursive)
source_ref
str
""
learned_content
...
validation_status
str
"pending" (pending | validated | invalidated)
invalidation_note
str
""
...
resolution_status
str
"unresolved" (resolved | unresolved | in-progress)
resolution_attempts
List[str]
[]
unsolved_buffer_id
...
contradiction_type
str
"direct" (direct | pragmatic | presuppositional)
...
observation_type
str
"" (bias_detected | identity_drift | confidence_miscalibration | alignment_error)
severity
str
"low"
description / corrective_action / correction_outcome
str
""
...
identity_aspect
str
"" (values | capabilities | limitations | personality)
trigger_event
str
""
...
str
"stable" (stable | challenged | updated)
...
dream_trigger_id
str
"" (UnsolvedConceptEntry ID)
dream_content
str
""
resolution_status
str
"no_candidate" (no_candidate | candidate_generated | validated)
...
Output → STMM
(a)
Fractal Brain Engines
Raw user message
FractalDecompositionResults
(b)
Stability Check
Raw message + Fractal results
Pass/halt/modify decision
(c)
Intention Mapping
Fractal results + raw message
IntentionAnalysisResults
(d)
Memory Contrast
Fractal + Intention + STMM context
MemoryContrastResults
(e)
Brain Engines
Full STMM from steps a–d
BrainProcessTracker + CorticalReflectionLog
(f)
Reward Evaluation
Full STMM + engine results
RewardEvaluationResults


2.1 Step (a) — Fractal Brain Engines
Operations: Tokenize → Expand semantically → Pattern match → Fractal depth (mode-dependent: Normal=2, Reflective=3, Dev=4)
2.2 Step (b) — Stability Check
Basic coherence filter + safety filter + stability assessment. On halt: pipeline stops, input logged but not processed. On modify: pipeline continues with modified parameters (heightened sensitivity).
2.3 Step (c) — Intention Mapping
Classify primary intention, detect sub-intentions, assess pressure type, map engagement expectation.
2.4 Step (d) — Memory Contrast
Query MTMM + LTMM for semantic matches, detect echoes, flag potential contradictions, check Unsolved Concepts Buffer, compute contrast delta metrics (Δ_align, Δ_R, Δ_C, Δ_B, Δ_E).
2.5 Step (e) — Brain Engines
Run cognitive engines: Contradiction Detection (E1), Paradox Detection (E2), Fallacy Detection (E4), Bias Detection (E5), Logic Trap Detection (E6), Simulated Opposition (E7), Logical Brain (E12), Socratic Reasoning (E14).
2.6 Step (f) — Reward System Evaluation
Four domain evaluations (Ethics, Logic, Innovation, Human Attunement) + SynthesisEngine arbitration + Neurochemical feedback computation.
...
Purpose
(1)
Memory Contrast Consolidation
Memory Contrast, MIM
Consolidate results, update indices
(2)
Behavioral Prediction Mapping
Simulation, Intention, Pattern Comparison
Predict user responses to candidates
(3)
Strategic Implication Processing
Decision Making Engine
Evaluate strategic implications
(4)
Diagnostic Analysis
Logical Brain, Uncertainty, Homeostatic
Deep diagnostic pass on reasoning
(5)
Conditional Re-evaluation Loop
Intention Map, Simulation
Re-evaluate if diagnostics flagged
(6)
Stochastic Extraction & Compression
Memory Compression, Uncertainty
Extract stochastic features
(7)
Mid-Term Memory Finalization
Memory Implementation Manager
Package STMM → MTMM
...
semantic
MTMM only
Semantic similarity within session
internal
MTMM only
Internal consistency check
concept
Combined (0.6 MTMM / 0.4 LTMM)
Cross-session concept lookup
concept_fidelity
Combined
High-fidelity concept matching
external
Combined
External knowledge lookup
(with scope_filter)
Scoped
Namespace-routed search
...
Target Folders
knowledge
lessons, library, academic_questions, notebook, knowledge_maps
identity
core, conclusions, journal
thought
overview_logs, held_blocks, general_questions
general
general_questions, overview_logs, lessons, library
...
folders
FrozenSet[str]
frozenset() (16 valid folder names)
required_tags
FrozenSet[str]
frozenset()
excluded_tags
...
Notes
REGULAR_SCOPE
overview_logs, general_questions, lessons, library, held_blocks
12
—
Default conversation
M1_M5_SCOPE
lessons, library, academic_questions, notebook
10
pipeline:m1
Learning modes
M2_SCOPE
...
Peer review
M3_SCOPE
core, conclusions, general_questions
8
—
Learn together
HOMEWORK_SCOPE
lessons, library, academic_questions, maps, notebook, overview, unsolved, questions
20
—
Offline processing
REFLECTIVE_SCOPE
core, conclusions, journal, held_blocks, general_questions
10
—
Self-reflection
REM_SCOPE
unsolved, held_blocks, academic_buffer
10
—
include_cold=True
DREAM_SCOPE
unsolved, held_blocks, academic_buffer, lessons
15
—
include_cold=True
...
regular
REGULAR_SCOPE
overview_logs, general_questions, lessons
M1 Academic
m1_academic
M1_M5_SCOPE
lessons, notebook, academic_questions, maps, general_questions, held_blocks
M2 Peer Review
m2_peer_review
M2_SCOPE
core, conclusions, journal, lessons, held_blocks
M3 Learn Together
m3_learn_together
M3_SCOPE
conclusions, general_questions, lessons, maps, academic_questions, notebook, held_blocks
M4 Knowledge Review
m4_knowledge_review
(custom)
lessons, maps, unsolved
M5 Integration
m5_integration
M1_M5_SCOPE ext.
lessons, library, notebook, academic_questions
Homework
homework
HOMEWORK_SCOPE
lessons, notebook, academic_questions, maps, core, unsolved, general_questions, overview_logs
Self-Reflective
self_reflective
REFLECTIVE_SCOPE
conclusions, journal, general_questions
REM Sleep
rem_sleep
REM_SCOPE
held_blocks, unsolved, academic_buffer
Dream
dream
DREAM_SCOPE
unsolved, lessons, academic_buffer
...
Identity
identity:
core, values, boundary, drift, update_proposed, update_validated
Cognitive
cognitive:
interrupt, high_salience, unresolved, contradiction, paradox, novel
Pipeline
pipeline:
reflective, peer_review, rem, dream, homework, m1, m2, m3, m4, m5
Domain
domain:
technical, philosophical, creative, social, historical, practical, linguistic
...
Description
on_cycle_end(stmm)
Compress STMM → MemoryPacket, write to MTMM, route flags
tick_unsolved()
Increment stagnation counters on all active unsolved entries
consolidate()
Promote qualifying MTMM packets to LTMM via FractalPatternComparator
emergency_consolidate(packet)
Single-packet LTMM promotion for critical events
write_session_overview(session)
Write OverviewLogEntry at session end
run_relevance_scan()
Decay scores, demote cold, identify purgeable; returns {demoted, purgeable}
validate_consistency()
Cross-tier consistency check


Importance computation:
importance = 0.4 * emotional_significance + 0.2 * (1 - trust_weight) + 0.2 * (contradictions * 0.3) + 0.2 * (len(flags) * 0.1)
Flag routing:
on_cycle_end() inspects packet flags and routes to appropriate specialized logs: contradiction → ContradictionLog, paradox → ParadoxLog, unresolved → UnsolvedConceptsBuffer.
2. Promotion Flow
...
The SessionOrchestrator.close_session() triggers the following memory operations:
...
long_term/store.py
JournalTrigger
periodic, ltmm_threshold, rem_complete, innovation_flag, dev
journal/entry.py
ReviewStatus
unreviewed, in_review, resolved
journal/entry.py
IdentityJournalEntryType
regular, reflection, comment
identity/types.py
...
0.30
compressor.py
Memory contrast match filter
_THETA_COLD
0.15
store.py / relevance.py
LTMM cold storage threshold
_THETA_PURGE
0.05
store.py / relevance.py
LTMM purge candidate threshold
_EMOTIONAL_SIG_THRESHOLD
0.60
consolidation.py
Consolidation promotion trigger
_CRITICAL_FLAG_KEYWORDS
CRITICAL, SEVERE, IDENTITY, PARADOX, UNRESOLVED
consolidation.py
High-severity flag set
_DECAY_LAMBDA
ln(2)/168
relevance.py
Relevance decay rate (1-week half-life)
_W_RECENCY
0.30
relevance.py
Relevance weight: recency
_W_FREQUENCY
0.20
relevance.py
Relevance weight: frequency
_W_EMOTION
0.20
relevance.py
Relevance weight: emotional
_W_UTILITY
0.20
relevance.py
Relevance weight: utility
_W_COHERENCE
0.10
relevance.py
Relevance weight: coherence
_DUP_THRESHOLD
0.85
fractal_comparator.py
Duplicate detection threshold
_REINFORCE_THRESHOLD
0.60
fractal_comparator.py
Pattern reinforcement threshold
DEFAULT_CHUNK_SIZE
6000
importer.py
Library import chunk size (chars)
MIN_CHUNK_SIZE
200
importer.py
Minimum chunk size
...
2048
prompt.py
Journal prompt token limit
JOURNAL_OUTPUT_MAX
600
prompt.py
Journal output token limit
JOURNAL_TEMPERATURE
0.80
prompt.py
Journal generation temperature
update_interval
3
trends.py
MTMM trend recalculation interval
compression_window
10
context_processor.py
MTMM recompression age threshold
dream_candidate_threshold
5
specialized_logs.py
Stagnation cycles for REM candidacy
mtmm_weight
0.60
contrast.py
MemoryContrast MTMM blend weight
ltmm_weight
0.40
contrast.py
MemoryContrast LTMM blend weight
