...
##
## Addendum B.4.1: toast feedback on override/reset, E17 parameter learning toggle.
##
...
func _on_dev_data(key: String, data: Dictionary) -> void:
	if key == "reward/override_weights/result":
		_show_toast("Domain weights overridden", _Toast.Level.SUCCESS)
		refresh()
		return
	if key == "reward/reset_weights/result":
		_show_toast("Domain weights reset to static", _Toast.Level.INFO)
		refresh()
		return
	if key == "reward/e17_learning/result":
		var enabled : bool = data.get("enabled", false)
		_show_toast("E17 parameter learning %s" % ("enabled" if enabled else "disabled"),
			_Toast.Level.SUCCESS)
		return
	if key != "reward":
		return
...
func _on_request_failed(path: String, error: Dictionary) -> void:
	if "/dev/reward" not in path:
		return
	_show_toast("Failed to load reward data: HTTP %s" % str(error.get("http_code", "?")),
		_Toast.Level.ERROR)
...
func _build_body(parent: VBoxContainer) -> void:
	...
	# E17 Parameter Learning toggle (B.4.1)
	var e17_sep := HSeparator.new()
	parent.add_child(e17_sep)
	var e17_row := HBoxContainer.new()
	e17_row.add_theme_constant_override("separation", 8)
	parent.add_child(e17_row)
	_e17_toggle = CheckButton.new()
	_e17_toggle.text = "E17 Parameter Learning"
	_e17_toggle.tooltip_text = "When enabled, E17 (Reward-Based Learning) continuously adjusts domain weights based on prediction error feedback."
	_e17_toggle.focus_mode = Control.FOCUS_NONE
	_e17_toggle.add_theme_font_size_override("font_size", 11)
	_e17_toggle.add_theme_color_override("font_color", Color(0.75, 0.78, 0.82))
	_e17_toggle.toggled.connect(_on_e17_toggled)
	e17_row.add_child(_e17_toggle)
	var e17_hint := Label.new()
	e17_hint.text = "Adjusts domain weights via prediction error"
	e17_hint.add_theme_color_override("font_color", Color(0.40, 0.40, 0.45))
	e17_hint.add_theme_font_size_override("font_size", 10)
	e17_row.add_child(e17_hint)
...
func _populate(d: Dictionary) -> void:
	...
	# E17 toggle state
	var e17_active : bool = d.get("e17_learning_enabled", false)
	_e17_toggle.set_pressed_no_signal(e17_active)
...
func _on_e17_toggled(pressed: bool) -> void:
	ZADOSClient.post_dev("reward/e17_learning", {"enabled": pressed})
