...
func _on_request_failed(path: String, error: Dictionary) -> void:
	if "/memory/ltmm/identity" not in path:
		return
	var msg := "HTTP %s — %s" % [
		str(error.get("http_code", "?")),
		str(error.get("body", "Connection failed")).left(120)]
	var target_list : VBoxContainer = null
	var section_name := "Identity"
	if "core" in path:
		target_list = _core_list; section_name = "Core Memories"
	elif "hardcoded" in path:
		target_list = _hardcoded_list; section_name = "Hardcoded"
	elif "development" in path:
		target_list = _dev_list; section_name = "Development"
	elif "alignment" in path:
		target_list = _alignment_list; section_name = "Alignment"
	if target_list:
